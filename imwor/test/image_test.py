...
from imwor.image import color_hex_to_dec_tuple, Image
...
def get_image_rotate_cases():
    criteria_combinations = _make_combinations([dict(values=[[90, 180, 315], [1, 0]], fields=["degree", "expand"])])
    cases = []
    for criteria in criteria_combinations:
        cases.append(_criteria_to_rotate_case("test1.jpg", criteria))
    criteria_combinations = _make_combinations([dict(values=[["auto"], [0]], fields=["degree", "expand"])])
    for criteria in criteria_combinations:
...
def get_image_region_cases():
    criteria_combinations = _make_combinations(
        [dict(values=[["150,150,100,100", "200,175,50,50"]], fields=["rect"])])
    cases = []
    for criteria in criteria_combinations:
...
def get_image_chained_cases():
    criteria_combinations = _make_combinations(
        [dict(values=[[("resize", "rotate"), ("rotate", "resize")], [(150, 75), (75, 150)], [90]],
              fields=["operation", "size", "degree"]),
         dict(values=[[("resize", "region", "rotate")], [(150, 75), (75, 150)], ["5,5,65,65"], [90]],
              fields=["operation", "size", "rect", "degree"]),
         dict(values=[[("region", "resize", "rotate")], [(150, 75), (75, 150)], ["50,50,150,150"], [90]],
              fields=["operation", "size", "rect", "degree"])])
    cases = []
    for criteria in criteria_combinations:
...
def get_image_exif_cases():
    criteria_combinations = _make_combinations(
        [dict(values=[[1, 0], [300], [300]], fields=["preserve_exif", "width", "height"])])
    cases = []
    for criteria in criteria_combinations:
...
class ImageTest(unittest.TestCase):
    ...
    def test_invalid_rectangle(self):
        invalid_rectangles = ["", None, "100,100,200", "100,200,300,400.5", "0,-1,100,100", "100,100,-100,-100"]
    ...
    def test_valid_default_options_with_empty_values(self):
        opts = dict(mode=None, filter=None, background=None, optimize=None, position=None, quality=None,
                    progressive=None, preserve_exif=None)
    ...
    def test_bad_mode(self):
        self.assertRaises(errors.ModeError, Image.validate_options, dict(mode="foo"))

    def test_bad_filter(self):
        self.assertRaises(errors.FilterError, Image.validate_options, dict(filter="foo"))

    def test_bad_format(self):
        self.assertRaises(errors.FormatError, Image.validate_options, dict(format="foo"))

    def test_bad_background_invalid_number(self):
        self.assertRaises(errors.BackgroundError, Image.validate_options, dict(background="foo"))

    def test_bad_background_wrong_length(self):
        self.assertRaises(errors.BackgroundError, Image.validate_options, dict(background="0f"))
        self.assertRaises(errors.BackgroundError, Image.validate_options, dict(background="0f0f0"))
        self.assertRaises(errors.BackgroundError, Image.validate_options, dict(background="0f0f0f0f0"))
    ...
    def test_bad_position_ratio(self):
        self.assertRaises(errors.PositionError, Image.validate_options, dict(position="1.2,5.6"))
    ...
    def test_bad_quality_invalid_number(self):
        self.assertRaises(errors.QualityError, Image.validate_options, dict(quality="foo"))

    def test_bad_quality_invalid_range(self):
        self.assertRaises(errors.QualityError, Image.validate_options, dict(quality=101))
        self.assertRaises(errors.QualityError, Image.validate_options, dict(quality=-1))

    def test_bad_optimize_invalid_bool(self):
        self.assertRaises(errors.OptimizeError, Image.validate_options, dict(optimize="b"))

    def test_bad_preserve_exif_invalid_bool(self):
        self.assertRaises(errors.PreserveExifError, Image.validate_options, dict(preserve_exif="b"))

    def test_bad_progressive_invalid_bool(self):
        self.assertRaises(errors.ProgressiveError, Image.validate_options, dict(progressive="b"))

    def test_bad_retain_invalid_range(self):
        self.assertRaises(errors.RetainError, Image.validate_options, dict(retain=101))
        self.assertRaises(errors.RetainError, Image.validate_options, dict(retain=-1))

    def test_color_hex_to_dec_tuple(self):
        tests = [["fff", (255, 255, 255)], ["ccc", (204, 204, 204)], ["abc", (170, 187, 204)],
                 ["ffffff", (255, 255, 255)], ["cccccc", (204, 204, 204)], ["abcdef", (171, 205, 239)],
                 ["fabc", (170, 187, 204, 255)], ["0abc", (170, 187, 204, 0)], ["8abc", (170, 187, 204, 136)],
                 ["80abcdef", (171, 205, 239, 128)], ["ffabcdef", (171, 205, 239, 255)],
    ...
    def test_save_failure(self):
        ...
        self.assertRaises(errors.ImageSaveError, lambda: img.save(format="webp"))

    def _assert_expected_resize(self, case):
        with open(case["source_path"], "rb") as f:
            img = Image(f).resize(case["width"], case["height"], mode=case["mode"], background=case.get("background"),
                                  filter=case.get("filter"), position=case.get("position"), retain=case.get("retain"))
...
def _get_simple_criteria_combinations():
    return _make_combinations(
        [dict(values=[Image.MODES, [(400, 300), (300, 300), (100, 200)]], fields=["mode", "size"]),
         dict(values=[["crop"], [(200, 100)], ["center", "face"]], fields=["mode", "size", "position"])]
    )


def _get_example_criteria_combinations():
    return [dict(mode="adapt", width=500, height=400, retain=80),
            dict(mode="adapt", width=500, height=400, retain=99, background="ccc"),
            dict(mode="clip", width=500, height=400), dict(mode="crop", width=500, height=400),
            dict(mode="fill", width=500, height=400, background="ccc"), dict(mode="scale", width=500, height=400)
            ]


def _get_advanced_criteria_combinations():
    return _make_combinations(
        [dict(values=[["adapt"], [(125, 75)], [99, 80, 60, 40]], fields=["mode", "size", "retain"]),
         dict(values=[["fill"], [(125, 75)], ["F00", "cccccc"]], fields=["mode", "size", "background"]),
         dict(values=[["crop"], [(125, 75)], Image.POSITIONS], fields=["mode", "size", "position"]),
         dict(values=[["crop"], [(125, 75)], ["0.25,0.75", "0.25,0.25"]], fields=["mode", "size", "position"]),
         dict(values=[["crop"], [(125, 75)], Image.FILTERS], fields=["mode", "size", "filter"]),
         dict(values=[["crop"], [(125, 75)], [50, 75, 90, "keep"]], fields=["mode", "size", "quality"]),
         dict(values=[["crop"], [(125, 75)], [1, 0]], fields=["mode", "size", "optimize"]),
         dict(values=[["crop"], [(125, 75)], [1, 0]], fields=["mode", "size", "progressive"]),
         dict(values=[Image.MODES, [(125, None), (None, 125)]], fields=["mode", "size"]),
         dict(values=[["crop"], [(125, 75)], Image.FORMATS], fields=["mode", "size", "format"])])


def _get_transparent_criteria_combinations():
    return _make_combinations(
        [dict(values=[["fill"], [(75, 125)], ["1ccc", "a0cccccc"]], fields=["mode", "size", "background"])])
...
def _make_combinations(choices):
    ...
    for choice in choices:
        for a in list(itertools.product(*choice["values"])):
            combo = dict(list(zip(choice["fields"], a)))
            if "size" in combo:
...
def _criteria_to_resize_case(filename, criteria):
    ...
    fields = ["mode", "filter", "quality", "background", "position", "optimize", "progressive", "retain"]
    opts_desc = "-".join(["{}={}".format(x, str(criteria.get(x))) for x in fields if criteria.get(x)])
    expected = "{}-{}x{}{}.{}".format(m.group(1), criteria.get("width") or "", criteria.get("height") or "",
                                      ("-{}".format(opts_desc)) if opts_desc else "",
...
def _criteria_to_rotate_case(filename, criteria):
    ...
    opts_desc = "-".join(["{}={}".format(x, str(criteria.get(x))) for x in fields if criteria.get(x)])
    expected = "{}-rotate{}.{}".format(m.group(1), ("-{}".format(opts_desc)) if opts_desc else "",
                                       criteria.get("format") or m.group(2))
    case["expected_path"] = os.path.join(EXPECTED_DATADIR, expected)
    return case
...
def _criteria_to_region_case(filename, criteria):
    ...
    fields = ["rect"]
    opts_desc = "-".join(["{}={}".format(x, str(criteria.get(x))) for x in fields if criteria.get(x)])
    expected = "{}-region{}.{}".format(m.group(1), ("-{}".format(opts_desc)) if opts_desc else "",
                                       criteria.get("format") or m.group(2))
    case["expected_path"] = os.path.join(EXPECTED_DATADIR, expected)
    return case
...
def _criteria_to_chained_case(filename, criteria):
    ...
    fields = ["degree", "rect"]
    opts_desc = "-".join(["{}={}".format(x, str(criteria.get(x))) for x in fields if criteria.get(x)])
    expected = "{}-chained-{}-{}x{}{}.{}".format(m.group(1), ",".join(criteria.get("operation", [])),
                                                 criteria.get("width") or "", criteria.get("height") or "",
                                                 ("-{}".format(opts_desc)) if opts_desc else "", m.group(2))
    case["expected_path"] = os.path.join(EXPECTED_DATADIR, expected)
    return case
...
def _criteria_to_exif_case(filename, criteria):
    ...
    opts_desc = "-".join(["{}={}".format(x, str(criteria.get(x))) for x in fields if criteria.get(x) is not None])
    expected = "{}-exif-{}.{}".format(m.group(1), opts_desc or "", m.group(2))
    case["expected_path"] = os.path.join(EXPECTED_DATADIR, expected)
    return case
