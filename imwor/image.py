...
_positions_to_ratios = {
    "top-left": (0.0, 0.0), "top": (0.5, 0.0), "top-right": (1.0, 0.0), "left": (0.0, 0.5), "center": (0.5, 0.5),
    "right": (1.0, 0.5), "bottom-left": (0.0, 1.0), "bottom": (0.5, 1.0), "bottom-right": (1.0, 1.0), "face": None
}

_orientation_to_rotation = {3: 180, 6: 90, 8: 270}

_filters_to_pil = {"antialias": PIL.Image.ANTIALIAS, "bicubic": PIL.Image.BICUBIC, "bilinear": PIL.Image.BILINEAR,
                   "nearest": PIL.Image.NEAREST
                   }

_formats_to_pil = {"gif": "GIF", "jpg": "JPEG", "jpeg": "JPEG", "png": "PNG", "webp": "WEBP"}


class Image(object):
    FILTERS = list(_filters_to_pil.keys())
    FORMATS = list(_formats_to_pil.keys())
    MODES = ["adapt", "clip", "crop", "fill", "scale"]
    POSITIONS = list(_positions_to_ratios.keys())

    _DEFAULTS = dict(background="fff", expand=False, filter="antialias", format=None, mode="crop", optimize=False,
                     position="center", quality=90, progressive=False, retain=75, preserve_exif=False)
    ...
    def __init__(self, stream):
        ...
        if self.img.format.lower() not in self.FORMATS:
            raise errors.ImageFormatError("Unknown format: {}".format(self.img.format))
    ...
    def validate_dimensions(width, height):
        if not width and not height:
            ...
            raise errors.DimensionsError("Invalid width: {}".format(width))
        elif height and not str(height).isdigit():
            raise errors.DimensionsError("Invalid height: {}".format(height))
    ...
    def resize(self, width, height, **kwargs):
        """
        Resizes the image to the supplied width/height. Returns the instance. Supports the following optional keyword
        arguments:
        ...
        position - The position used to crop: see Image.POSITIONS for pre-defined positions or a custom position ratio
        retain - The minimum percentage of the original image to retain when cropping
        """
    ...
    def rotate(self, deg, **kwargs):
        """
        Rotates the image clockwise around its center.  Returns the instance. Supports the following optional keyword
        arguments:
    ...
    def _get_face_rectangles(self):
        ...
        return cv.HaarDetectObjects(cvim, self._get_face_classifier(), cv.CreateMemStorage(0),
                                    1.3,  # Scale factor
                                    4,  # Minimum neighbors
                                    0,  # HAAR Flags
                                    (20, 20))
    ...
    def _normalize_options(options):
        ...
        for k, v in list(options.items()):
            if v is not None:
                opts[k] = v
        opts["pil"] = dict(filter=_filters_to_pil.get(opts["filter"]), format=_formats_to_pil.get(opts["format"]),
...
def color_hex_to_dec_tuple(color):
    """Converts a color from hexadecimal to decimal tuple, color can be in the following formats:
    3-digit RGB, 4-digit ARGB, 6-digit RGB and 8-digit ARGB.
    """
