...
define("port", help="run on the given port", type=int, default=9999)
...
class ImworApplication(tornado.web.Application):
    def __init__(self, **kwargs):
        settings = dict(
            ...
            allowed_operations=set(options.allowed_operations or ImageHandler.OPERATIONS),
...
class ImageHandler(tornado.web.RequestHandler):
    ...
    _FORMAT_TO_MIME = {"gif": "image/gif", "jpeg": "image/jpeg", "jpg": "image/jpeg", "png": "image/png",
                       "webp": "image/webp"}
    ...
    def fetch_image(self):
        ...
        if self.settings.get("implicit_base_url") and urlparse(url).hostname is None:
            url = urljoin(self.settings.get("implicit_base_url"), url)
        client = tornado.httpclient.AsyncHTTPClient(
            max_clients=self.settings.get("max_requests"))
        try:
            resp = yield client.fetch(url, request_timeout=self.settings.get("timeout"),
            ...
            logger.warn("Fetch error for {}: {}".format(self.get_argument("url"), str(exc)))
    ...
    def write_error(self, status_code, **kwargs):
        ...
        if isinstance(err, errors.ImworError):
            self.set_header("Content-Type", "application/json")
            resp = dict(status_code=status_code, error_code=err.get_code(), error=err.log_message)
    ...
    def _set_headers(self, headers, file_format):
        if file_format and any((self.get_argument("fmt"), self.settings.get("format"),
                                self.settings.get("content_type_from_image"))):
            self.set_header("Content-Type", self._FORMAT_TO_MIME.get(file_format.lower()))
    ...
    def _get_resize_options(self):
        return self._get_options(
            dict(mode=self.get_argument("mode"), filter=self.get_argument("filter"), position=self.get_argument("pos"),
                 background=self.get_argument("bg"), retain=self.get_argument("retain")))
    ...
    def _get_save_options(self):
        return self._get_options(
            dict(format=self.get_argument("fmt"), optimize=self.get_argument("opt"), quality=self.get_argument("q"),
                 progressive=self.get_argument("prog"), preserve_exif=self.get_argument("exif")))
...
def start_server(app=None):  # pragma: no cover
    ...
    server = tornado.httpserver.HTTPServer(app if app else ImworApplication())
